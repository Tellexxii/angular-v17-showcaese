import {ApplicationRef, Component, computed, effect, signal, untracked, WritableSignal} from '@angular/core';
...
import {asyncScheduler, combineLatest, debounceTime, map, Observable, of, tap, throttleTime} from "rxjs";
import {toObservable, toSignal} from "@angular/core/rxjs-interop";
import {MatSnackBar} from "@angular/material/snack-bar";
import {FormsModule, ReactiveFormsModule} from "@angular/forms";
import {MatFormFieldModule} from "@angular/material/form-field";
import {MatInputModule} from "@angular/material/input";
import {getActiveConsumer, producerAccessed, REACTIVE_NODE, SIGNAL} from "@angular/core/primitives/signals";
...
export class TodoComponent {
    ...
    constructor(private todosStore: TodoStoreService, private _snackBar: MatSnackBar) {
        ...
        const test = toSignal(of(1))

        //to Observable

        this.todosView$ = combineLatest([
            toObservable(this.todos),
            toObservable(this.searchInput).pipe(
                debounceTime(500, asyncScheduler))
        ]).pipe(
            map(([todos, searchInput]) => searchInput === ''
                ? todos
                : todos.filter(todo => todo.title.toLowerCase().includes(searchInput.toLowerCase())))
        )
